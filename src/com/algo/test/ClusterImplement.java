...
public class ClusterImplement {
	public HashMap[] symbolLocation;
	public static HashMap dataFinance;
	...
	public ClusterImplement(HashMap dataFinance2) {
		...
		int xx=1;
	      ...
	      for  (int i = 0; i < dataFinance.size(); i++) {  // Fill first 6 rows with random values.
	    	  //se = biff.get(i);
				...
				//a[2] = (Double) dd[1];//r.nextDouble()*100*xx;
				listS.add(a[0]);
				listS.add(a[1]);
				//listS.add(a[2]);
...
// show cluster association
		    ...
		    //		 //symbolLocation[k] = new HashMap();
		    	// }
		    	 symbolLocation[k].put(items[m].toUpperCase().toString(),dp);
		    	 
		    	 xresult.append(k+"~").append(items[m].toUpperCase()+"@point="+m+" associated with cluster:"+k).append("");
		    	 xresult.append(NL);
			...
			xresult = new StringBuffer();
			for(DataPoint o :Centers.getArray())
			{
	  ...
	  public JScrollPane prepareMain() {
		...
		public static void qqq(String[] args) {
	        ...
	        cluster.runner();
	       // System.out.println(SetEnv.DATALOG);
